    ...
    <div className="FinalResult">
      ...
      <Row>
        <Col className="text-center">
          <h5>Session name: {SessionInfo?.sessionName}</h5>
          <h6>
            Date and time: {SessionInfo?.date} [{SessionInfo.time}]
          </h6>
          <h6>
            Game play time: {gameTime.hour > 0 ? `${gameTime.hour}:` : ""}
            {Math.abs(parseInt(startTime[1]) - parseInt(stopTime[1]))}m:
            {Math.abs(parseInt(startTime[2]) - parseInt(stopTime[2]))}s
          </h6>
        </Col>
      </Row>
      <br />
